...
<em>This project implements and compares two machine learning models, <b>Random Forest</b> and <b>Multi-Layer Perceptron (MLP)</b>, to predict the presence of diabetes in patients. The analysis uses a comprehensive clinical dataset with 18 features following WHO standards. The system focuses on reliable classification, evaluated through rigorous Stratified K-Fold validation and metrics like Accuracy, Recall, and AUC.</em>
</div>
...
* **`diabetes.csv`**: The dataset used for training and testing. It contains 18 clinical features (such as Age, BMI, Blood Pressure) collected from 4303 patients.

## Project Highlights

* **Dataset**: Utilizes the *Diabetes Dataset with 18 Features* (sourced from Kaggle), providing a robust set of predictors compared to smaller standard datasets.
* **Methodology**:
...
* **Models Compared**:
    * **Random Forest**: Optimized for high Recall (minimizing false negatives), crucial for medical screening.
    * **MLP (Neural Network)**: Optimized for high Precision and overall stability.
